/* Rights Checker — intermediate screen, unique to this tool.
 *
 * Provides window.RIGHTS_CHECKLIST(combo, ctx) to decision-tree-core.js. Renders
 * the 5 standard consumer rights as a ✓ / ✗ checklist based on the combination's
 * "rights" field, then a "Continue" button that advances to the shared Action Plan
 * screen (window.ACTION_PLAN, see action-plan-renderer.js) for the same
 * problem+country — the same component the Wizard renders directly. This screen
 * itself is NOT a result screen; it's an extra step before the shared result.
 */
(function () {
  ...
  window.RIGHTS_CHECKLIST = function (combo, ctx) {
    var esc = ctx.esc;
    ...
    var html =
      ...
      '<p class="wiz-step">' + esc(ctx.stepLabel || 'Step 3 of 4') + '</p>' +
      ...
      '<button type="button" class="rc-cta" data-next="result">See your action plan →</button>' +
