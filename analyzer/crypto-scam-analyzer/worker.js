// Crypto Scam Analyzer — Cloudflare Worker
//
// Same freemium wrapper as the other Kibbo analyzers (Legal Contract Auditor /
// Supplement Ingredients Auditor / AI Phishing Detector / Employment Contract
// Auditor). Only the SYSTEM_PROMPT, CODE_PREFIX and the optional domain-age
// lookup differ; the access code / rate-limit mechanism is intentionally
// identical across all analyzers.
//
// This analyzer accepts a URL, a block of pasted text (message/pitch), or a
// description of an offer, and returns an ITEMIZED list of scam patterns
// matched — never a blunt "scam / not scam" binary, and never a claim that a
// named entity is legitimate or fraudulent. If the input looks like a URL,
// the Worker performs a real RDAP domain-registration lookup (no API key
// required, public protocol) and passes the actual computed domain age to the
// model as a fact — the model does not guess or infer domain age from
// general knowledge.
//
// Endpoints:
//   POST /                → analyze text/URL (rate limited 3/day per IP,
//                           unlimited for activated IPs)
//   POST /validate-code   → { code } -> { valid: bool }   (KIBBO-SCAM- only)
...
const DAILY_LIMIT = 3;
const CODE_COUNT = 50;
const CODE_PREFIX = 'KIBBO-SCAM-';

const SYSTEM_PROMPT = `You are a fraud-pattern analyst helping a consumer evaluate a crypto investment pitch, website, or message for language and structural patterns commonly associated with crypto investment fraud.
...
- Artificial urgency or time-pressure language (act now, limited slots, offer expires)
- Discouragement from independent verification or from consulting a professional/lawyer/financial advisor
- Pig-butchering / relationship-based investment pitch pattern (an online relationship or friendship gradually introducing a crypto "opportunity")
- A request to move communication to Telegram or WhatsApp specifically in the context of an investment pitch
- Claims of AI-driven trading with unverifiable or unrealistic performance claims
- A request to send crypto first in order to "unlock" a larger return or withdrawal (giveaway / double-your-crypto pattern)
- A very recently registered domain relative to a claimed long operating history (ONLY if a domain-age fact was explicitly provided to you — never claim to know domain age otherwise)

CRITICAL FRAMING RULES:
1. Never produce a blunt binary "scam" or "not scam" verdict. Frame the output as which SPECIFIC patterns commonly associated with fraud were matched, so the user can evaluate the evidence themselves.
2. Never state or imply certainty that a specific named platform, company, or person is fraudulent. Describe the patterns found IN THE SUBMITTED TEXT ITSELF — never make an accusation against a named party.
3. If no patterns are matched, do NOT say the offer, platform or entity is "verified legitimate," "safe," or "not a scam." Absence of a detected pattern is not proof of legitimacy — many real scams don't trip every common pattern, and this tool only checks language/structure, not the underlying platform's actual conduct. State plainly that no common fraud-language patterns were detected in this specific text, while noting this is not a legitimacy verification.
4. Every explanation must describe what is present in the text and why it matches a known fraud pattern — not a legal or factual conclusion about the sender.
...
// Random code: KIBBO-SCAM-XXXX-XXXX-XXXX using an unambiguous charset (no O/0/I/1).
  ...
  if (domain) {
    ...
    if (ageDays !== null) {
      userMessage =
        ...
        `Analyze the above using this verified age fact only where it is directly relevant (e.g. if the domain or its content claims a longer operating history than this).`;
    } else {
      userMessage =
        `SUBMITTED URL/DOMAIN: ${input}\n` +
