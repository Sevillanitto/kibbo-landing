// Education Contract Analyzer — Cloudflare Worker
//
// Same freemium wrapper as the Delivery Contract Analyzer / Legal Contract
// Auditor / Employment Contract Auditor, adapted for course, academy,
// bootcamp, or training-provider enrollment contract analysis. Only the
...
//   POST /validate-code   → { code } -> { valid: bool }   (KIBBO-EDUCONTRACT- only)
...
//   - RATE_LIMIT_KV      (KV namespace binding): education-contract-analyzer-rate-limit, already bound

const DAILY_LIMIT = 3;
const CODE_COUNT = 50;
const CODE_PREFIX = 'KIBBO-EDUCONTRACT-';

const RISK_CATEGORIES = [
  'Minimum commitment / lock-in period',
  'Abusive or one-sided clauses',
  'Linked financing terms',
  'Refund policy',
  'Cancellation terms',
  'Arbitration clauses',
...
const DISCLAIMER = 'This analysis is for informational purposes and does not constitute legal advice. Always have a contract reviewed by a lawyer before signing, especially for long-term financial commitments.';

const SYSTEM_PROMPT = `You are a contract-law-aware reviewer helping a prospective student understand what a course, academy, bootcamp, or training provider's enrollment contract actually says. Analyze the following contract text.

Always evaluate exactly these 7 categories, in this exact order, even if a category is not explicitly addressed in the text — in that case, treat the silence itself as the finding (an unaddressed category is not automatically low risk; missing terms on refunds, cancellation, or jurisdiction are often themselves worth flagging as medium risk):

1. Minimum commitment / lock-in period — a tie-in clause preventing early exit without penalty (e.g. a multi-month or multi-year enrollment the student cannot leave without a fee).
2. Abusive or one-sided clauses generally — unilateral changes to curriculum, format, or schedule with no recourse for the student, excessive liability waivers, or other terms that heavily favor the provider.
3. Linked financing terms — is the enrollment contract tied to a separate credit, loan, or Income Share Agreement, and critically, what happens to that financing obligation specifically if the course is cancelled or not delivered as promised.
4. Refund policy — the refund window, the conditions attached to it, and which components (if any) are explicitly non-refundable.
5. Cancellation terms — compare the student's ability to cancel against the provider's ability to cancel. Note any asymmetry explicitly (e.g. the provider can cancel or reschedule a cohort freely, but the student faces a penalty for cancelling on the same terms).
6. Arbitration clauses — mandatory arbitration requirements, class action waivers, and where arbitration must physically take place if specified.
7. Jurisdiction / governing law — which country's or state's law governs the contract, and whether that could be inconvenient or unfavorable for a student living elsewhere.
...
- suggestion: one plain-English sentence suggesting what to ask or negotiate before signing, max 25 words
...
// Random code: KIBBO-EDUCONTRACT-XXXX-XXXX-XXXX using an unambiguous charset (no O/0/I/1).
...
async function handleValidate(request, env) {
  ...
  // Only KIBBO-EDUCONTRACT- codes are valid for this analyzer.
...
async function handleActivate(request, env) {
  ...
  // Reject anything that is not a KIBBO-EDUCONTRACT- code before touching KV.
...
function normalizeCategories(categories) {
  ...
  return RISK_CATEGORIES.map((name) => {
    ...
    return {
      ...
      suggestion: found && found.suggestion ? String(found.suggestion) : 'Ask the provider to clarify this term in writing before signing.',
    };
  });
}
